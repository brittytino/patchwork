package com.brittytino.essentials.viewmodels
...
import com.brittytino.essentials.domain.registry.SearchRegistry
import com.brittytino.essentials.data.repository.SettingsRepository
import com.brittytino.essentials.data.repository.UpdateRepository
import com.brittytino.essentials.domain.model.AppSelection
import com.brittytino.essentials.domain.model.NotificationApp
import com.brittytino.essentials.domain.model.NotificationLightingColorMode
import com.brittytino.essentials.domain.model.NotificationLightingSide
import com.brittytino.essentials.domain.model.NotificationLightingStyle
import com.brittytino.essentials.domain.model.SearchableItem
import com.brittytino.essentials.domain.model.UpdateInfo
import com.brittytino.essentials.services.CaffeinateWakeLockService
import com.brittytino.essentials.services.NotificationLightingService
import com.brittytino.essentials.services.tiles.ScreenOffAccessibilityService
import com.brittytino.essentials.services.receivers.SecurityDeviceAdminReceiver
import com.brittytino.essentials.utils.AppUtil
...
import com.brittytino.essentials.utils.UpdateNotificationHelper
...
class MainViewModel : ViewModel() {
    ...
    val defaultTab = mutableStateOf(com.brittytino.essentials.domain.DIYTabs.ESSENTIALS)
    ...
    val snoozeChannels = mutableStateOf<List<com.brittytino.essentials.domain.model.SnoozeChannel>>(emptyList())
    ...
    fun check(context: Context) {
        ...
        if (isRootEnabled.value) {
            isRootAvailable.value = com.brittytino.essentials.utils.RootUtils.isRootAvailable()
            isRootPermissionGranted.value = com.brittytino.essentials.utils.RootUtils.isRootPermissionGranted()
        } else {
    ...
    fun triggerNotificationLighting(context: Context) {
        ...
        try {
            val intent = Intent(context, com.brittytino.essentials.services.NotificationLightingService::class.java).apply {
    ...
    fun setDefaultTab(tab: com.brittytino.essentials.domain.DIYTabs, context: Context) {
    ...
    fun setBluetoothDevicesEnabled(enabled: Boolean, context: Context) {
        ...
        val intent = Intent(context, com.brittytino.essentials.services.widgets.BatteriesWidgetReceiver::class.java).apply {
    ...
    fun freezeAllAuto(context: Context) {
        viewModelScope.launch(Dispatchers.IO) {
            com.brittytino.essentials.utils.FreezeManager.freezeAll(context)
        }
    }

    fun unfreezeAllAuto(context: Context) {
        viewModelScope.launch(Dispatchers.IO) {
            com.brittytino.essentials.utils.FreezeManager.unfreezeAll(context)
        }
    }

    fun freezeAllManual(context: Context) {
        viewModelScope.launch(Dispatchers.IO) {
            com.brittytino.essentials.utils.FreezeManager.freezeAllManual(context)
        }
    }

    fun unfreezeAllManual(context: Context) {
        viewModelScope.launch(Dispatchers.IO) {
            com.brittytino.essentials.utils.FreezeManager.unfreezeAllManual(context)
        }
    }

    fun launchAndUnfreezeApp(context: Context, packageName: String) {
        viewModelScope.launch(Dispatchers.IO) {
            val isFrozen = com.brittytino.essentials.utils.FreezeManager.isAppFrozen(context, packageName)
            if (isFrozen) {
                com.brittytino.essentials.utils.FreezeManager.unfreezeApp(context, packageName)
    ...
    fun generateBugReport(context: Context): String {
        ...
        return com.brittytino.essentials.utils.LogManager.generateReport(context, settingsJson)
    }
